...
class FluidSimulation {
  ...
  _initTextures() {
    ...
    this._fboVelocity = new alfrid.FboPingPong(size, size, oSettings);
    this._fboDensity = new alfrid.FboPingPong(size, size, oSettings);
    this._fboPressure = new alfrid.FboPingPong(size, size, oSettings);
  ...
  _initDrawCalls() {
    ...
    //	draw calls
    this._drawAdvect = new alfrid.Draw()
    ...
    this._drawDivergence = new alfrid.Draw()
    ...
    this._drawClear = new alfrid.Draw()
      .useProgram(vs, fsClear)
      .setMesh(mesh)
      .setClearColor(0, 0, 0, 1)
      .uniform("uDissipation", "float", this.settings.PRESSURE_DISSIPATION);

    this._drawJacobi = new alfrid.Draw()
      .useProgram(vs, fsJacobi)
      .setMesh(mesh)
      .setClearColor(0, 0, 0, 1)
      .uniform("uTexelSize", "vec2", this._texelSize);

    this._drawGradient = new alfrid.Draw()
      .useProgram(vs, fsGradientSub)
      .setMesh(mesh)
      .setClearColor(0, 0, 0, 1)
      .uniform("uTexelSize", "vec2", this._texelSize);

    this._drawSplat = new alfrid.Draw().useProgram(vs, fsSplat).setMesh(mesh);
    this._drawAdd = new alfrid.Draw().useProgram(vs, fsAdd).setMesh(mesh);
  }
